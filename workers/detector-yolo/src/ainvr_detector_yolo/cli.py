"""Command line interface for the detector worker."""
...
from .events import normalize_detection_events, normalize_error_event
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="AI-NVR YOLO detection worker")
    ...
    parser.add_argument(
        "--warmup",
        action="store_true",
        help="Warm up the backend before processing frames",
    )
    return parser
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    for frame in frames:
        ...
        if batch.error is not None:
            event = normalize_error_event(batch, worker_config=config)
            print(json.dumps(event, sort_keys=True))
            continue
        for event in normalize_detection_events(batch, worker_config=config):
            print(json.dumps(event, sort_keys=True))
    return 0
